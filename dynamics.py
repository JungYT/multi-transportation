...
class Link(BaseEnv):
    def __init__(self, length, anchor, uvec_bound):
        ...
        #     )
        # ))
        self.uvec = BaseSystem(np.vstack((0., 0., 0.)))
        self.omega = BaseSystem(np.vstack((0., 0., 0.)))
        self.len = length
        self.anchor = anchor
...
class MultiQuadSlungLoad(BaseEnv):
    ...
    def reset(self, load_pos_des, load_att_des, fixed_init=True):
        super().reset()
        if fixed_init:
    ...
    def set_dot(self, t, quad_att_des, f_des):
        ...
        for i, (link, quad) in enumerate(
            ...
            m = quad.mass
            R = quad.dcm.state
            u = -f_des[i] * R.dot(np.vstack((0., 0., 0.)))
            R0_omega_square_rho = R0.dot(omega_hat_square.dot(rho))
    ...
    def logger_callback(self, t, y, i, t_hist, ode_hist,
        ...
        for i, link in enumerate(self.links.systems):
            ...
            quad_vel.append(
                ...
                - link.len*hat(links[f'link{i:02d}']['omega'])
            )
            quad_att.append(
                np.array(rot.dcm2angle(quads[f'quad{i:02d}']['dcm']))[::-1]
            )
            anchor_pos.append(load['pos'] + load['dcm'].dot(link.anchor))
        distance = self.check_collision(quad_pos)
        return dict(time=t, **states, moment=M, quad_att_des=quad_att_des,
                    f_des=f_des, quad_pos=quad_pos, quad_vel=quad_vel,
                    quad_att=quad_att, anchor_pos=anchor_pos,
                    distance=distance)
