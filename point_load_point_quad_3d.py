...
center_pos_init = np.vstack((-2.0, -2.0, -1.0))
swing_angle_init = np.vstack([0])
rotation_angle_init = np.vstack([0])
load_vel_init = np.vstack((0.0, 0.0, 0.0))
center_posxy_rand = [-5, 5]
center_posz_rand = [-1, -10]
swing_angle_rand = [-np.pi/3, np.pi/3]
rotation_angle_rand = [-np.pi, np.pi]
...
class Load(BaseEnv):
    ...
    def __init__(self):
        ...
        self.vel = BaseSystem(load_vel_init)
...
class PointMassLoadPointMassQuad3D(BaseEnv):
    ...
    def reset(self, fixed_init=False):
        ...
        if fixed_init:
            ...
            load_pos, link0, link1 = self.convert_state_learn2dynamics(state_learn_init)
            self.load.pos.state = load_pos
            self.links.link00.link.state = link0
            self.links.link01.link.state = link1
        else:
            center_pos_init = np.vstack(
                (np.random.uniform(low=center_posxy_rand[0], high=center_posxy_rand[1]),
                 np.random.uniform(low=center_posxy_rand[0], high=center_posxy_rand[1]),
                 np.random.uniform(low=center_posz_rand[0], high=center_posz_rand[1]))
            )
            swing_angle_init = np.vstack(
                [np.random.uniform(low=swing_angle_rand[0], high=swing_angle_rand[1])]
            )
            rotation_angle_init = np.vstack(
                [np.random.uniform(low=rotation_angle_rand[0], high=rotation_angle_rand[1])]
            )
            state_learn_init = [
                center_pos_init,
                swing_angle_init,
            ...
            self.links.link00.link.state = link0
            self.links.link01.link.state = link1
        x = np.hstack((load_pos, swing_angle_init, rotation_angle_init, load_vel))
        return x

    def convert_state_learn2dynamics(self, state_learn):
        center_pos, swing_angle, rotation_angle, *_ = state_learn
        load_pos = center_pos \
            + np.vstack((
                link_len*np.cos(between_angle)*np.sin(swing_angle)*np.sin(rotation_angle),
                -link_len*np.cos(between_angle)*np.sin(swing_angle)*np.cos(rotation_angle),
                link_len*np.cos(between_angle)*np.cos(swing_angle)
            ))
        #load_vel = self.load.vel.state
        quad0_pos = center_pos \
            + np.vstack((
                link_len*np.sin(between_angle)*np.cos(rotation_angle),
                link_len*np.sin(between_angle)*np.sin(rotation_angle),
                0
            ))
    ...
    def observe(self):
        ...
        z_unit = np.vstack((1.0, 0.0, 0.0))
        swing_angle = np.arccos(
            np.clip(
                np.dot(center2load/np.linalg.norm(center2load), z_unit),
                -1.0, 1.0)
        )
...
def main(dis_factor, actor_learning_rate, critic_learning_rate, softupdate_const,
    ...
    env = PointMassLoadPointMassQuad3D(reward_weight)
