...
class Payload(BaseEnv):
    def __init__(self):
        super().__init__()
        self.pos = BaseSystem(np.vstack((0., 0., 0.)))
...
class Link(BaseEnv):
    def __init__(self):
        super().__init__()
        self.q = BaseSystem(np.vstack((0., 0., 0.)))
        self.w = BaseSystem(np.vstack((0., 0., 0.)))
...
class Env(BaseEnv):
    ...
    def reset(self, fixed_init=False):
        ...
        if fixed_init:
            phi = np.pi / 4
            self.payload.pos.state = np.vstack((0., 0., -1.))
            self.links.link00.q.state = np.vstack((np.sin(phi), 0.,
                                                   np.cos(phi)))
            self.links.link01.q.state = np.vstack((-np.sin(phi), 0.,
                                                   np.cos(phi)))
        else:
    ...
    def step(self, action, reference):
        ...
        *_, done = self.update(u=u)
        ...
        if self.payload.pos.state[2] > 0 \
        or np.linalg.norm(quad_pos[0] - quad_pos[1]) < self.bubble:
            done = True
        r = self.reward(action, reference)

        info = {
            "action_converted": action_converted
        }
        self.logger.record(**info)

        x = self.observation()

        return x, r, done, quad_pos, self.payload.pos.state
    ...
    def observation(self):
        ...
        theta = np.arctan2(self.links.link00.q.state[2],
                           self.links.link00.q.state[0])
    ...
    def action_convert_to_pos(self, u):
        ...
        for i in range(self.n_quad):
            ...
            phi = u[2]
            pos_x = R * np.cos(-np.pi / 2 + (-1)**(i+1) * phi)
            pos_y = 0
            pos_z = R * np.sin(-np.pi / 2 + (-1)**(i+1) * phi)
...
class ActorNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # scaling
        x = x * torch.Tensor([self.action_pos_bound,
                              self.action_pos_bound, np.pi/4]) \
...
def main():
    ...
    for n_epi in range(config["simulation"]["n_epi"]):
        ...
        while True:
            ...
            xn, r, done, *_ = env.step(u, reference)
            r_train = (r + 100) / 100
        ...
        if (n_epi * config["simulation"]["n_test"]) % \
            config["simulation"]["n_epi"] == 0 and n_epi != 0:
            for test_epi in range(config["simulation"]["n_test_epi"]):
                x = env.reset(fixed_init=config["simulation"]["fixed_init"])
                #x = env.reset()
                while True:
                    u = agent.action(x, use="behavior")
                    x, r, done, quad_pos, load_pos = env.step(u, reference)
                    r_train = (r + 100) / 100
                    score += r_train
                    """
                    ...
                    if test_epi == 0 and config["simulation"]["animation"]:
                        ...
                        plt.cla()
                        plt.axis([-10, 10, -1, 15])
                        plt.plot(config["simulation"]["reference"][0],
                                 config["simulation"]["reference"][2], 'go')
                        plt.plot([x[0], quad_pos[0][0]],
                                 [-x[1], -quad_pos[0][2]], 'k', linewidth=0.5)
                        plt.plot([x[0], quad_pos[1][0]],
                                 [-x[1], -quad_pos[1][2]], 'k', linewidth=0.5)
...
def figure(score, data_save):
    ...
    ax2[0].plot(time, pos[:,0], 'r--',
                time, config["simulation"]["reference"][0]*np.ones(len(time)),
                'b')
    ax2[1].plot(time, pos[:,2], 'r--',
                time, config["simulation"]["reference"][2]*np.ones(len(time)),
                'b')
